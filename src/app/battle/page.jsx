...
export default function BattlePage() {
  ...
  useEffect(() => {
    ...
    // playerIdが指定されていない、または既に使用されている場合
    if (!playerIdParam || existingPlayers.includes(playerIdNum)) {
      // 利用可能なplayerIdを取得
      const availablePlayerId = getAvailablePlayerId();
      if (availablePlayerId === null) {
        alert('このルームは満員です（最大4人）');
        router.push('/battle/join');
        return;
      }
      // URLを更新して利用可能なplayerIdに変更
      router.replace(`/battle?room=${roomId}&player=${availablePlayerId}`);
      return;
    }
  ...
  }, [roomId, playerId, playerIdParam, router]);
